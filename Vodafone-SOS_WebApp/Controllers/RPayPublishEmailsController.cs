...
{
    public class RPayPublishEmailsController : PrimaryController
    {
        // GET: RPayPublishEmails
        IRPayPublishEmailsRestClient RestClient = new RPayPublishEmailsRestClient();
        public ActionResult Index()
        {
            System.Web.HttpContext.Current.Session["Title"] = "Manage Account Payable emails";
            return View();
        }
        int CompanyId = Convert.ToInt32(System.Web.HttpContext.Current.Session["CompanyId"]);
        public JsonResult GetEmailIDsDropdownData()
        {
            var ApiData = RestClient.GetEmailIDsDropdownData(CompanyId);
            return Json(ApiData, JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        //[ControllerActionFilter]
        public JsonResult EditPayPublishEmails(object[] GridData)
        {
            try
            {
                foreach (string Data in GridData)
                {
                    ...
                    var model = new RPayPublishEmailsViewModel { CompanyId = CompanyId, Id = Convert.ToInt32(GridArray[0]), Department = GridArray[1], EmailIds = GridArray[2] };
                    RestClient.Add(model, null);
                }
               
                var OutputJson = new { ErrorMessage = "Create Pay Publish Emails", PopupMessage = "", RedirectToUrl = "/RPayPublishEmails/Index" };
                ...
                if (!string.IsNullOrEmpty(ex.Data["ErrorMessage"] as string))
                {
                    ...
                    switch ((int)ex.Data["ErrorCode"])
                    {
                        ...
                        case (int)ExceptionType.Type2:
                            ...
                            var OutputJson2 = new { ErrorMessage = ex.Data["ErrorMessage"] as string, PopupMessage = "", RedirectToUrl = "/RPayPublishEmails/Index" };
                        ...
                        case (int)ExceptionType.Type4:
                            var OutputJson4 = new { ErrorMessage = "", PopupMessage = ex.Data["ErrorMessage"] as string, RedirectToUrl = "/RPayPublishEmails/Index" };
                            return Json(OutputJson4, JsonRequestBehavior.AllowGet);
                    }
                    var OutputJson = new { ErrorMessage = ex.Data["ErrorMessage"] as string, PopupMessage = "", RedirectToUrl = "/RPayPublishEmails/Index" };
                    return Json(OutputJson, JsonRequestBehavior.AllowGet);
                }
                else
                {
       ...
       // [ControllerActionFilter]
        public JsonResult DeletePayPublishEmails(int Id)
        {
            try
            {
                RestClient.Delete(Id, null, CompanyId);
                var OutputJson = new { ErrorMessage = "Create Pay Publish Emails ", PopupMessage = "", RedirectToUrl = "" };
                return Json(OutputJson, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
